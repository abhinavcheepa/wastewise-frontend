...
const fetchWasteResult = () =>
new Promise(res => setTimeout(() =>
res(MOCK_ITEMS[Math.floor(Math.random()*MOCK_ITEMS.length)]), 1100));
...
const getTier = pts => TIERS.find(t => pts>=t.min && pts<=t.max) || TIERS[0]; /* ─── Confetti
    ...
    cur=cur<target?Math.min(cur+step,target):Math.max(cur-step,target); setD(cur); if(cur===target) clearInterval(t);
        ...
        const scan = () => {
        setLoading(true); setResult(null); setNewPts(null);
        fetchWasteResult().then(r => {
        const pts = W[r.waste_type].pts;
        setResult(r);
        setLoading(false);
        setAnimKey(k=>k+1);
        setNewPts(pts);
        ...
        emoji:W[r.waste_type].emoji, pts,
